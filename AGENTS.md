...
This repository is intentionally small and task-focused. `buttonclick.py` is the main Python entrypoint and contains the Camoufox automation logic. `buttonclick.html` can be used to inspect the page structure of the target site `https://camoufox.com/tests/buttonclick`. `README.md` is minimal project metadata, and `notes.txt` holds working notes rather than production code. Keep generated files such as `__pycache__/` out of commits, and keep the local virtual environment in `venv/`.
